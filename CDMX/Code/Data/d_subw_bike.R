...
# Bikestops
# bikestops = read.csv('Data\\Geo\\estaciones_ecobici_sist_anterior.csv')

bikestops = read.csv('Data\\Geo\\estaciones_ecobici_sist_anterior.csv') %>% 
  st_as_sf(coords=c("longitud","latitud"), crs=4326) # remember x=lon and y=lat

# Subte 
metro = st_read('Data\\Geo\\Metro\\STC_Metro_estaciones_utm14n.shp')
...
metro = st_transform(metro, utm_crs)


dists = as.matrix(st_distance(bikestops,metro))
...
while(i < length){
  ...
  saveRDS(get(name) ,paste0('Data/Distances/Intervals/',name2,'.rds'))
...
open = readRDS('Data/Final/open_raw.rds')
open = open[1:480,]

names(open) =seq(as.Date('2010-02-01'),as.Date('2019-12-01'),by='1 month')


# dates relevant to our study (pre-covid and starting from where we have bike tris data)
dates = seq(as.Date('2010-02-01') , as.Date('2019-12-31') ,by = '1 day' )
...
for(i in 1:length(dates)){
  idx = which(bikestops$num_cicloestacion %in% open[,which(names(open)==dates2[i])][!is.na(open[,which(names(open)==dates2[i])])]) 
...
filos = list.files('Data/Distances/Intervals/')

for(n in filos){
  nom = substr(n,1,nchar(n)-4)
  temp = readRDS(paste0('Data/Distances/Intervals/',n))
  assign(nom,temp)
}
...
saveRDS(final_0_400,'Data/Distances/Intervals x Open/m_0_400.rds')
...
saveRDS(final_400_3000,'Data/Distances/Intervals x Open/m_400_3000.rds')
...
saveRDS(final_0_200,'Data/Distances/Intervals x Open/m_0_200.rds')
...
saveRDS(final_800_3000,'Data/Distances/Intervals x Open/m_800_3000.rds')
...
saveRDS(final_800_2600,'Data/Distances/Intervals x Open/m_800_2600.rds')
...
saveRDS(final_800_2800,'Data/Distances/Intervals x Open/m_800_2800.rds')
...
saveRDS(final_1400_2800,'Data/Distances/Intervals x Open/m_1400_2800.rds')
...
saveRDS(final_1000_2000,'Data/Distances/Intervals x Open/m_1000_2000.rds')
...
saveRDS(final_1200_3200,'Data/Distances/Intervals x Open/m_1200_3200.rds')
